...
public class TabelaEleitor {
    ...
    public boolean adicionarEleitor(Eleitor novoEleitor) {
        ...
        while ((!achouEspacoLivre) && (!deuErro)) {
            ...
            i++;
            if ((i == this.limite) && (!achouEspacoLivre))
                deuErro = true;
        }
    ...
    public void listarEleitores() {
        for (int i = 0; i < eleitores.length; i++) {
            if (eleitores[i] != null)
                System.out.printf("%nCódigo: %d%nNome: %s%nSituação: %d", eleitores[i].getCodigo(),
                        eleitores[i].getNome(), eleitores[i].getSituacao());
        }
    }

    public boolean carregarArquivoEleitores(String localArquivo) {
        // para se divertir
        return localArquivo.equals("");
    }
}
